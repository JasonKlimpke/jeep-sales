...
class FetchJeepTest {
  ...
  class TestsThatDoNotPolluteTheApplicationContext extends FetchJeepTestSupport{
     ...
     */
    @Test
    void testThatAnErrorMessegeIsReturnedWhenAnUnknownTrimIsSupplied() {
      //Given: a valid model, trim and URI
      JeepModel model = JeepModel.WRANGLER;
      ...
      // Then: a a not found (404) status code is returned
      assertThat(response.getStatusCode()).isEqualTo(HttpStatus.NOT_FOUND);
      
      // And: an error message is returned
      Map<String, Object> error = response.getBody();
      
      assertErrorMessegeValid(error, HttpStatus.NOT_FOUND);
    }
   ...
   void testThatAnErrorMessegeIsReturnedWhenAnInvalidValueIsSupplied(
     ...
     assertErrorMessegeValid(error, HttpStatus.BAD_REQUEST);
    }
   ...
   static Stream<Arguments> parametersForInvalidInput() {
     //@formatter:off
     return Stream.of(
         arguments("WRANGLER", "@#$%^&&%", "Trim contains non-alpha-numeric charicters."),
   ...
   */
  @Test
  void testThatAnUnplannedErrorResaultsInA500Status() {
    //Given: a valid model, trim and URI
    JeepModel model = JeepModel.WRANGLER;
    String trim = "Invalid";
    String uri = 
    ...
    assertErrorMessegeValid(error, HttpStatus.INTERNAL_SERVER_ERROR);
  }
